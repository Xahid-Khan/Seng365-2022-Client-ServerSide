...
const create = async(auction: postAuction): Promise<number> => {
    const query = `INSERT INTO \`auction\` (title, description, category_id , seller_id, reserve, end_date) VALUES (?)`;
    const [result] = await getPool().query(query, [[auction.title, auction.description, auction.categoryId, auction.sellerId, auction.reserve, auction.endDate]])
