...
const update = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if(req.body.hasOwnProperty("email") && (!req.body.email.includes('@') || req.body.email.length>128)){
            res.statusMessage = `Invalid email`;
            res.status(400).send();
            return;
        }
        if(req.body.hasOwnProperty("firstName") && (req.body.firstName.length<1 || req.body.firstName.length>64)) {
            res.statusMessage = `Invalid length of first name`;
            res.status(400).send();
            return;
        }
        if(req.body.hasOwnProperty("lastName") && (req.body.lastName.length<1 || req.body.lastName.length>64)) {
            res.statusMessage = `Invalid length of last name`;
            res.status(400).send();
            return;
        }
        ...
        if(req.body.hasOwnProperty("password")) {
            if(req.body.hasOwnProperty("currentPassword") && req.body.password.length>=1 && req.body.password.length<=256 && req.body.currentPassword.length<=256 && await passwords.compare(req.body.currentPassword, user.password))
